...
namespace STL
{
	...
	Matrix4f Matrix4f::Translation(float x, float y, float z)
	{
		...
		result.Get(3, 0) = x;	  result.Get(3, 1) = y;		result.Get(3, 2) = z;     result.Get(3, 3) = 1.0f; // 열 우선 행렬로 쓰기 위해서 전치됨. (원래(행 우선 행렬)같으면 세로로 x,y,z가 (0,3), (1,3), (2,3), (3,3)에 있어야 함)
	...
	Matrix4f Matrix4f::Rotation(Quaternionf rotation)
	{
	...
	Matrix4f Matrix4f::RotationX(float angle)
	{
		...
		angle *= MathHelper::Deg2Rad;

		float cosX = std::cosf(angle);
		float sinX = std::sinf(angle);

		rx.Get(0, 0) = 1.0f;  rx.Get(0, 1) = 0.0f;  rx.Get(0, 2) = 0.0f;   rx.Get(0, 3) = 0.0f; //마지막번째는 안쓰고 1줄이 없다고 생각하고 좌상+cos 우상+sin 좌하-sin 우하+cos 로 곱한다.
	...
	Matrix4f Matrix4f::RotationY(float angle)
	{
		...
		ry.Get(1, 0) = 0.0f;	ry.Get(1, 1) = 1.0f;  ry.Get(1, 2) = 0.0f;   ry.Get(1, 3) = 0.0f; //마지막번째는 안쓰고 2줄이 없다고 생각하고 좌상+cos 우상-sin 좌하+sin 우하+cos로 곱한다.
	...
	Matrix4f Matrix4f::RotationZ(float angle)
	{
		...
		rz.Get(2, 0) = 0.0f;  rz.Get(2, 1) = 0.0f;   rz.Get(2, 2) = 1.0f;  rz.Get(2, 3) = 0.0f; //마지막번째는 안쓰고 3줄이 없다고 생각하고 좌상+cos 우상+sin 좌하-sin 우하+cos로 곱한다.
	...
	Matrix4f Matrix4f::LookAt(Vector3f& position, Vector3f& target, Vector3f& up)
	{
		...
		//result.Get(0, 0) = right.x;	    result.Get(0, 1) = right.y;	    result.Get(0, 2) = right.z;	result.Get(0, 3) = -x;
		//result.Get(1, 0) = upDir.x;	    result.Get(1, 1) = upDir.y;	    result.Get(1, 2) = upDir.z;	result.Get(1, 3) = -y;
		//result.Get(2, 0) = forward.x;		result.Get(2, 1) = forward.y;		result.Get(2, 2) = forward.z;	result.Get(2, 3) = -z;
		//result.Get(3, 0) = 0;			result.Get(3, 1) = 0;			result.Get(3, 2) = 0;			result.Get(3, 3) = 1.0f;

		return result;
	}

	Matrix4f Matrix4f::Perspective(float fov, float width, float height,
		float zNear, float zFar)
	{
		float aspect = width / height;
		float radians = fov / 2.0f;

		float H = 1 / std::tanf(radians);
		float W = H / aspect;
	...
	float& Matrix4f::Get(const uint32 row, uint32 col)
	{
		assert(row <= 3 && col <= 3);
